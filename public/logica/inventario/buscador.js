...
============================
*/
const tbody = document.querySelector('.contenedor_datos');
...
function buscar_dato(x){
	...
	if (tbody) {
		...
		//Por medio del nombre, codigo o el id
		...
		for (var i = 0; i < datos.length; i++){
			...
			if (datos[i].id == x || datos[i].nombre == x || datos[i].codigo == x){
				console.log("encontrado = " + datos[i].id + " " + datos[i].nombre + " " + datos[i].codigo);
